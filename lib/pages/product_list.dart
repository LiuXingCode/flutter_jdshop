...
class _ProductListPageState extends State<ProductListPage> {
  ...
  _getProductListData() async {
    ...
    if (_keywords != null) {
      api =
          "${Config.domain}api/plist?search=$_keywords&page=$_page&pageSize=$_pageSize&sort=$_sort";
    } else {
      api =
          "${Config.domain}api/plist?cid=$_cid&page=$_page&pageSize=$_pageSize&sort=$_sort";
    }
    var result = await Dio().get(api);
    var productData = ProductModel.fromJson(result.data);

    //判断是否有搜索数据
    setState(() {
      _hasData = productData.result.isNotEmpty || _page > 1;
    });
